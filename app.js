...
function showLocalScreenTile(stream) {
  removeScreenTile('me');
  const tile = buildScreenTile('screen-me', 'Você (compartilhando)', stream, true);
  screenGrid.appendChild(tile);
}

function showRemoteScreenTile(id, name, stream) {
  removeScreenTile(id);
  const tile = buildScreenTile('screen-' + id, name, stream, false);
  screenGrid.appendChild(tile);
}

function buildScreenTile(tileId, labelText, stream, muted) {
  ...
  tile.innerHTML = `
    ...
    <button class="expand-btn" title="Ver em tela cheia">⛶</button>
  `;
  ...
  video.ondblclick = goFullscreen;
  tile.querySelector('.expand-btn').onclick = goFullscreen;
...
// ---------- Tela cheia (janela inteira do site) ----------
